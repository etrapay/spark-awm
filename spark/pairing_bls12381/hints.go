...
func divG1Hint(nativeMod *big.Int, nativeInputs, nativeOutputs []*big.Int) error {
	return emulated.UnwrapHint(nativeInputs, nativeOutputs,
		func(mod *big.Int, inputs, outputs []*big.Int) error {
			var a, b, c bls12381.G1Affine

			a.X.SetBigInt(inputs[0])
			a.Y.SetBigInt(inputs[1])
			b.X.SetBigInt(inputs[2])
			b.Y.SetBigInt(inputs[3])

			c.X.Inverse(&b.X).Mul(&c.X, &a.X)
			c.Y.Inverse(&b.Y).Mul(&c.Y, &a.Y)
